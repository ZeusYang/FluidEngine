...
namespace Engine
{
	...
	void SphSolver3::onBeginAdvanceTimeStep(double timeStepInSeconds) 
	{
		...
		std::cout << "Building neighbor lists and updating densities took " << timer.durationInSeconds() << " seconds\n";
	}
	...
	void SphSolver3::onEndAdvanceTimeStep(double timeStepInSeconds) 
	{
		...
		std::cout << "Max density: " << maxDensity << " " << "Max density / target density ratio: "
			<< maxDensity / particles->targetDensity() << std::endl;
	}
	...
	double SphSolver3::computePressureFromEos(
		double density,
	...
	{
		return std::shared_ptr<SphSolver3>(new SphSolver3(
				_targetDensity, _targetSpacing, _relativeKernelRadius),
